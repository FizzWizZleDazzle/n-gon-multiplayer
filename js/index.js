    ...
    if (Object.keys(set).length !== 0) {
        ...
        for (const property in set) {
            ...
            if (property === "field") {
                let found = false
                let index
                for (let i = 0; i <localPlayer.m.fieldUpgrades.length; i++) {
                    if (set[property] ===localPlayer.m.fieldUpgrades[i].name) {
                        index = i;
                        found = true;
                        break;
                    }
            ...
            if (property === "molMode") {
               ...
               localPlayer.m.fieldUpgrades[i].description =localPlayer.m.fieldUpgrades[i].setDescription()
...
const build = {
    pixelDraw() {
        ...
        function loop() {
            ...
            if (simulation.paused &&localPlayer.m.alive) requestAnimationFrame(loop);
        }
            ...
            simulation.ephemera.push({
                name: "dmgDefBars", count: 0, do() {
                    if (!(m.cycle % 15)) { //4 times a second
                        const defense =localPlayer.m.defense()             //update defense bar
                        if (m.lastCalculatedDefense !== defense) {
                            document.getElementById("defense-bar").style.width = Math.floor(300 *localPlayer.m.maxHealth * (1 - defense)) + "px";
                           localPlayer.m.lastCalculatedDefense = defense
                        }
                        ...
                        if (m.lastCalculatedDamage !== damage) {
                           ...
                           localPlayer.m.lastCalculatedDamage = damage
                        }
                    }
                },
            })
    ...
    pauseGrid() {
        ...
        simulation.lastLogTime =localPlayer.m.cycle //hide in game console
...
<br><strong class='color-defense'>defense</strong>: ${(1 -localPlayer.m.defense()).toPrecision(5)} &nbsp; &nbsp; difficulty: ${(1 / simulation.dmgScale).toPrecision(4)}
...
${m.coupling ? `<br><span style = 'font-size:90%;'>` +localPlayer.m.couplingDescription(m.coupling) + `</span> from ${(m.coupling).toFixed(0)} ${powerUps.orb.coupling(1)}` : ""}
${botText}
<br>
...
${simulation.isCheating ? "<br><br><em>lore disabled</em>" : ""}
        ...
        if (tech.isPauseSwitchField && !simulation.isChoosing) {
            const style = localSettings.isHideImages ? `style="height:auto;"` : `style="background-image: url('img/field/${m.fieldUpgrades[m.fieldMode].name}${m.fieldMode === 0 ?localPlayer.m.fieldUpgrades[0].imageNumber : ""}.webp');"`
    ...
    choosePowerUp(index, type, isAllowed = false) {
        if (type === "gun") {
            ...
            if (m.fieldMode !== index) {
                document.getElementById("field-" +localPlayer.m.fieldMode).classList.remove("build-field-selected");
               localPlayer.m.setField(index)
...
<div>
    ...
    <div style="grid-column: 2;grid-row: 3/4;">
        ...
        for (let i = 0, len =localPlayer.m.fieldUpgrades.length; i < len; i++) {
            const style = localSettings.isHideImages ? hideStyle : `style="background-image: url('img/field/${m.fieldUpgrades[i].name}${i === 0 ?localPlayer.m.fieldUpgrades[0].imageNumber : ""}.webp');"`
    ...
    startExperiment() { //start playing the game after exiting the experiment menu
        ...
        if (!simulation.isCheating) {
            ...
            if (b.inventory.length !== 0 ||localPlayer.m.fieldMode !== 0) simulation.isCheating = true;
        }
...
window.addEventListener("keydown", function (event) {
    ...
    switch (event.code) {
        ...
        case input.key.pause:
            if (!simulation.isChoosing && input.isPauseKeyReady &&localPlayer.m.alive) {
                ...
                if (simulation.paused) {
                    ...
                    if (tech.isPauseSwitchField || simulation.testing) {
                        document.getElementById("pause-field").addEventListener("click", () => {
                            const energy =localPlayer.m.energy //save current energy
                            if (m.fieldMode === 4 && simulation.molecularMode < 3) {
                                simulation.molecularMode++
                               localPlayer.m.fieldUpgrades[4].description =localPlayer.m.fieldUpgrades[4].setDescription()
                            } else {
                               localPlayer.m.setField((m.fieldMode ===localPlayer.m.fieldUpgrades.length - 1) ? 1 :localPlayer.m.fieldMode + 1) //cycle to next field, skip field emitter
                                if (m.fieldMode === 4) {
                                    simulation.molecularMode = 0
                                   localPlayer.m.fieldUpgrades[4].description =localPlayer.m.fieldUpgrades[4].setDescription()
                                }
                            }
                           localPlayer.m.energy = energy //return to current energy
    ...
    if (simulation.testing) {
        if (event.key === "X")localPlayer.m.death(); //only uppercase
        switch (event.key.toLowerCase()) {
            ...
            case "8":
                ...
                break
            case "f":
                const mode = (m.fieldMode ===localPlayer.m.fieldUpgrades.length - 1) ? 0 :localPlayer.m.fieldMode + 1
               localPlayer.m.setField(mode)
                break
            case "g":
                b.giveGuns("all", 1000)
                break
            case "h":
                //localPlayer.m.health = Infinity
                if (m.immuneCycle === Infinity) {
                   localPlayer.m.immuneCycle = 0 //you can't take damage
                } else {
                   localPlayer.m.immuneCycle = Infinity //you can't take damage
                }

                //localPlayer.m.energy = Infinity
                // document.getElementById("health").style.display = "none"
                // document.getElementById("health-bg").style.display = "none"
                break
            case "n":
               localPlayer.m.addHealth(Infinity)
               localPlayer.m.energy =localPlayer.m.maxEnergy
                break
            case "y":
                tech.giveTech()
                break
...
//     }
//     for (let i = 1, len =localPlayer.m.fieldUpgrades.length; i < len; i++) {
//         const preloadLink = document.createElement("link");
//         preloadLink.href = "img/field/" +localPlayer.m.fieldUpgrades[i].name + ".webp";
//         preloadLink.rel = "preload";
//         preloadLink.as = "image";
//         document.head.appendChild(preloadLink);
//     }
...
if (!localSettings.isHideImages) {
    addEventListener("load", () => {
        ...
        for (let i = 1, len =localPlayer.m.fieldUpgrades.length; i < len; i++) urls.push("img/field/" +localPlayer.m.fieldUpgrades[i].name + ".webp")
        for (let i = 0, len = tech.tech.length; i < len; i++) {
...
function cycle() {
    ...
    if (elapsed > simulation.fpsInterval) { // if enough time has elapsed, draw the next frame
       ...
       localPlayer.m.cycle++; //tracks player cycles  //used to alow time to stop for everything, but the player
