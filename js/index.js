...
import { players, localPlayer, localPlayerNum} from "./multiplayer"
                ...
                name: "dmgDefBars", count: 0, do() {
                    if (!(m.cycle % 15)) { //4 times a second
                        ...
                        if (m.lastCalculatedDefense !== defense) {
                            document.getElementById("defense-bar").style.width = Math.floor(300 *players[localPlayerNum].m.maxHealth * (1 - defense)) + "px";
                           players[localPlayerNum].m.lastCalculatedDefense = defense
                        }
                        ...
                        if (m.lastCalculatedDamage !== damage) {
                           ...
                           players[localPlayerNum].m.lastCalculatedDamage = damage
                        }
                    }
                },
            })
    ...
    choosePowerUp(index, type, isAllowed = false) {
        if (type === "gun") {
            ...
            if (m.fieldMode !== index) {
                document.getElementById("field-" +players[localPlayerNum].m.fieldMode).classList.remove("build-field-selected");
               players[localPlayerNum].m.setField(index)
...
<div>
    ...
    <div style="grid-column: 2;grid-row: 3/4;">
        ...
        for (let i = 0, len =players[localPlayerNum].m.fieldUpgrades.length; i < len; i++) {
            const style = localSettings.isHideImages ? hideStyle : `style="background-image: url('img/field/${m.fieldUpgrades[i].name}${i === 0 ?players[localPlayerNum].m.fieldUpgrades[0].imageNumber : ""}.webp');"`
                ...
                if (players[localPlayerNum].simulation.paused) {
                    ...
                    if (tech.isPauseSwitchField || players[localPlayerNum].simulation.testing) {
                        document.getElementById("pause-field").addEventListener("click", () => {
                           ...
                           players[localPlayerNum].m.energy = energy //return to current energy
    ...
    if (players[localPlayerNum].simulation.testing) {
        ...
        switch (event.key.toLowerCase()) {
            ...
            case "f":
               ...
               players[localPlayerNum].m.setField(mode)
                break
            case "g":
                b.giveGuns("all", 1000)
                break
            case "h":
                //players[localPlayerNum].m.health = Infinity
                if (m.immuneCycle === Infinity) {
                   players[localPlayerNum].m.immuneCycle = 0 //you can't take damage
                } else {
                   players[localPlayerNum].m.immuneCycle = Infinity //you can't take damage
                }

                //players[localPlayerNum].m.energy = Infinity
                // document.getElementById("health").style.display = "none"
                // document.getElementById("health-bg").style.display = "none"
                break
            case "n":
               players[localPlayerNum].m.addHealth(Infinity)
               players[localPlayerNum].m.energy =players[localPlayerNum].m.maxEnergy
                break
            case "y":
                tech.giveTech()
                break
...
//     }
//     for (let i = 1, len =players[localPlayerNum].m.fieldUpgrades.length; i < len; i++) {
//         const preloadLink = document.createElement("link");
//         preloadLink.href = "img/field/" +players[localPlayerNum].m.fieldUpgrades[i].name + ".webp";
//         preloadLink.rel = "preload";
//         preloadLink.as = "image";
//         document.head.appendChild(preloadLink);
//     }
